...
sub configure_hook {
	...
	# set https redirection url list
    $self->{redirect_url_list} = &INetSim::Config::getConfigParameter("HTTPS_Redirect_URL_List");
    # set http dynamic paras match
    $self->{dynamic_paratoname} = &INetSim::Config::getConfigParameter("HTTPS_Dynamic_ParaToName");
    $self->{dynamic_paratomimetype} = &INetSim::Config::getConfigParameter("HTTPS_Dynamic_ParaToMIMETYPE");
    }
    else {
	...
	# set http redirection url list
    $self->{redirect_url_list} = &INetSim::Config::getConfigParameter("HTTP_Redirect_URL_List");
    # set http dynamic paras match
    $self->{dynamic_paratoname} = &INetSim::Config::getConfigParameter("HTTP_Dynamic_ParaToName");
    $self->{dynamic_paratomimetype} = &INetSim::Config::getConfigParameter("HTTP_Dynamic_ParaToMIMETYPE");
    }
...
sub process_request {
    ...
    $self->{http_response}{headers}{'Connection'} = "Close"; # this means that server does not keep connection
	...
	if ($self->{http_response}{status}) {
	    ...
	    # for adding 302 redirect action
   		...
   		# add 2017-03-31
   		# should add some configure key-value pairs like 'original_uri'=>'xxxx' and 'redirection_uri'=>'yyyy'
   		# the fullreq may be change to regex later
   		if (defined $self->{redirect_url_list} && defined $self->{redirect_url_list}{$fullreq}){
...
#    }
    ...
    # split dynamic part of request
    my @parts = split(/\?/, $self->{http_request}{request_uri});
    my $req = $parts[0];
    # support dynamic path matched
    if ($parts[1]){
    	my $req_dynamic_part = $parts[1] ;
    	$self->{http_request}{dynamic_part} = $req_dynamic_part;	
    }
...
sub send_http_response {
    ...
    # note that client may send http 1.0
...
sub read_fakefile {
    ...
    # check for static fakefile and dynamic paras
    # static option is prior to dynamic option
    # we assume no special need for indicating static and dynamic at the same time
    if (defined $self->{static_fakefile_pathtoname}{$self->{http_request}{pathfile}}) { #pathfile is the string after host section and befor the first ? in the url
    	...
    	my $matched_pattern = "";
    	if(($self->{http_request}{dynamic_part} ne "") &&  ($matched_pattern = $self->match_config_pattern($self->{http_request}{dynamic_part}))){
    		$fakefilename = $self->{fakeFileDir} . "/" . $self->{dynamic_paratoname}{$matched_pattern};
    		# set content-type
    		$self->{http_response}{headers}{'Content-Type'} = $self->{dynamic_paratomimetype}{$matched_pattern};
			&INetSim::Log::SubLog("[$rhost:$rport] info: Sending dynamic fake file configured for dynamic parameter '$self->{http_request}{dynamic_part}'.", $self->{servicename}, $$);
    	# default
    	}else {
...
sub match_config_pattern{
	my $self = shift;
	my $str = shift;
	foreach my $pattern (keys %{$self->{dynamic_paratoname}}){
		# find a pattern match the dynamic request
		if ($str =~ /$pattern/){
			return $pattern;
		}
	}
...
sub redirect_request{
    ...
    # construct redirection response header
    my $redirect_url = "";
    if (defined $self->{redirect_url_list}{$original_url}){
    	$redirect_url = $self->{redirect_url_list}{$original_url};
    }
    if ($redirect_url ne ""){
    	# set redirect url in section 'Location', and response status code to '302' 
    	$self->{http_response}{headers}{'Location'} = $redirect_url;
    	$self->{http_response}{headers}{'Content-Length'} = 0;

   	 	$self->set_response_status(302);
   	 	
   	 	# set filename to 'none' for logging
   		$self->{http_response}{filename} = "none";
   	 	
   	 	# log
   	 	&INetSim::Log::SubLog("[$rhost:$rport] info: Redirect $original_url to $redirect_url", $self->{servicename}, $$);
    	...
    	# should not happend
    	$self->error_exit("redirect url $redirect_url not find!");
    }
}
