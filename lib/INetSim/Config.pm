...
my @SERVICES = qw/smtp pop3 http ftp ntp dns ident daytime_tcp daytime_udp time_tcp time_udp echo_tcp echo_udp discard_tcp discard_udp chargen_tcp chargen_udp quotd_tcp quotd_udp tftp autofaketime finger dummy_tcp dummy_udp syslog irc port_detector/;
...
%ConfigOptions = (
		  ...
		  HTTP_Redirect_URL_List => {},			# added for url redirection <2017-04-01>
		  HTTP_Dynamic_ParaToName => {},		# added for dynamic request parameters match <2017-04-28>
		  HTTP_Dynamic_ParaToMIMETYPE => {},
		  ...
		  PDS_Default_Remote_Addr => "127.0.0.1",
		  PDS_Default_Bind_IP => "0.0.0.0",
		  PDS_Default_Bind_Port => 34456,
		  PDS_Remote_TCP_Port_List => [],
		  PDS_Remote_UDP_Port_List => [],
		  PDS_Next_Detection_Delay => 30,
		  PDS_Default_Send_Message => "hello server!\r\n\r\n",
		  PDS_ServiceName => undef
		  );
...
sub parse_config {
    ...
    my %http_dynamic_paratoname = ();		# added for dynamic para match <2017-04-28>
    my %http_dynamic_paratomimetype = ();
    my %http_redirect_url_list = ();		# added for url redirection <2017-04-01>
    ...
    my %https_dynamic_paratoname = ();		# added for dynamic para match <2017-04-28>
    ...
    my %tftp_options = ();
    my @pds_remote_tcp_port_list = ();		# added for port detection service
    my @pds_remote_udp_port_list = ();		# added for port detection service
...
#		}
	    ...
	    # HTTP_Redirect_URL_List
	    elsif($args[0] =~ /^http_redirect_url_list$/i){
	    	if (!$args[2]){
	    		&config_error("missing argument for http_redirect_url_list");
	    	}elsif($args[1] !~ /^http:\/\/[[:graph:]]+$/){
	    		&config_error("'$args[1]' is not a valid path");
	    	}elsif($args[2] !~ /^http:\/\/[[:graph:]]+$/){
	    		&config_error("'$args[2]' is not a valid path");
	    	}else{
	    		$http_redirect_url_list{$args[1]} = $args[2];
	    	}
	    }
	    
	    # added for dynamic request parameter match <2017-04-28>
	    # HTTP_Dynamic_ParaToName
	    elsif($args[0] =~ /^http_dynamic_paratoname/i){
			if (!$args[3]) {
			    &config_error("missing argument for http_dynamic_paratoname");
			}
			# args[1] should be a valid re expresion and just contains printable character
			eval{ qr/$args[1]/ };
			if (($args[1] !~ /^[[:graph:]]+$/) || ($args[1] =~ /\?/) || ($@)) {
			    &config_error("'$args[1]' is not a valid perl regex");
			}
			elsif ($args[2] !~ $RE_validFilename) {
			    &config_error("'$args[2]' is not a valid filename");
			    ...
			    $http_dynamic_paratoname{$args[1]} = $args[2];
			    $http_dynamic_paratomimetype{$args[1]} = $args[3];
			}
	    }
	    ...
	    # added for url redirection <2017-04-01>
	    # HTTPS_Redirect_URL_List
	    elsif($args[0] =~ /^https_redirect_url_list$/i){
	    	if (!$args[2]){
	    		&config_error("missing argument for https_redirect_url_list");
	    	}elsif($args[1] !~ /^https:\/\/[[:graph:]]+$/){
	    		&config_error("'$args[1]' is not a valid path");
	    	}elsif($args[2] !~ /^https:\/\/[[:graph:]]+$/){
	    		&config_error("'$args[2]' is not a valid path");
	    	}else{
	    		$https_redirect_url_list{$args[1]} = $args[2];
	    	}
	    }
	    
	   	# added for dynamic request parameter match <2017-05-02>
	    # HTTPS_Dynamic_ParaToName
	    elsif($args[0] =~ /^https_dynamic_paratoname/i){
			if (!$args[3]) {
			    &config_error("missing argument for http_dynamic_paratoname");
			}
			# args[1] should be a valid re expresion and just contains printable character
			...
			if (($args[1] !~ /^[[:graph:]]+$/) || ($args[1] =~ /\?/) || ($@)) {
			    ...
			    $https_dynamic_paratoname{$args[1]} = $args[2];
			    $https_dynamic_paratomimetype{$args[1]} = $args[3];
			}
	    }
		...
		####################################################
		# PDS remote addr to detect
		elsif ($args[0] =~ /^pds_default_remote_addr$/i){
			($args[1] =~ $RE_validIP) ? &setConfigParameter("PDS_Default_Remote_Addr", $args[1]) : &config_error("'$args[1]' is not a valid IP address");
		}
		
		# PDS default bind ip
		elsif($args[0] =~ /^pds_default_bind_ip$/i){
			($args[1] =~ $RE_validIP) ? &setConfigParameter("PDS_Default_Bind_IP", $args[1]) : &config_error("'$args[1]' is not a valid IP address");
		}
		
		# PDS default bind port
		elsif ($args[0] =~ /^pds_default_bind_port$/i){
			(($args[1] =~ /[\d]+/) && ($args[1] > 0) && ($args[1] < 65535)) ? &setConfigParameter("PDS_Default_Bind_Port", $args[1]) : &config_error("'$args[1]' is not a valid port number");
		}
		
		# PDS remote tcp port list to be detected
		elsif($args[0] =~ /^pds_remote_tcp_port_list$/i){
			if (!$args[1]){
				&config_error("missing argument for pds_remote_tcp_port_list");
			}else{
				my @port_list = split("," , $args[1]);
				my $re_port = qr/([\d]+)/;
				foreach (@port_list){
					if ($_ !~ /-/){
						if (($_ =~ $re_port) && ($_ > 0) && ($_ < 65535)){
							push(@pds_remote_tcp_port_list, $_);
						}else{
							&config_error("$_ is not a valid port");
						}
					}else{
						my ($p1, $p2) = split('-', $_);
						if ($p1 and ($p1 =~ $re_port) && ($p1 > 0) && ($p1 < 65535) && $p2 and ($p2 =~ $re_port) && ($p2 > 0) && ($p2 < 65535) && ($p1 <= $p2)){
							push(@pds_remote_tcp_port_list, ($p1..$p2));
						}else{
							&config_error("$_ is not a valid port range");
						}
					}
				}
			}
		...
		# PDS remote udp port list to be detected
		elsif($args[0] =~ /^pds_remote_udp_port_list$/i){
			if (!$args[1]){
				&config_error("missing argument for pds_remote_udp_port_list");
			}else{
				my @port_list = split("," , $args[1]);
				my $re_port = qr/([\d]+)/;
				foreach (@port_list){
					if ($_ !~ /-/){
						if (($_ =~ $re_port) && ($_ > 0) && ($_ < 65535)){
							push(@pds_remote_udp_port_list, $_);
						}else{
							&config_error("$_ is not a valid port");
						}
					}else{
						my ($p1, $p2) = split('-', $_);
						if ($p1 and ($p1 =~ $re_port) && ($p1 > 0) && ($p1 < 65535) && $p2 and ($p2 =~ $re_port) && ($p2 > 0) && ($p2 < 65535) && ($p1 <= $p2)){
							push(@pds_remote_udp_port_list, ($p1..$p2));
						}else{
							&config_error("$_ is not a valid port range");
						}
					}
		...
		# PDS pds next detection delay
		elsif ($args[0] =~ /^pds_next_detection_delay$/i){
			if (!$args[1]){
				&config_error("missing argument for pds_next_detection_delay");
			}elsif ($args[1] > 0 && $args[1] <= 3600){
				&setConfigParameter("PDS_Next_Detection_Delay", $args[1]);
			}else{
				&config_error("$args[1] is not a valid delay");
			}		
		}
		
		# PDS pds default send message
		# this maybe extend future, interactive with remote port using configured template
		elsif ($args[0] =~ /^pds_default_send_message$/i){
			if (!$args[1]){
				&config_error("missing argument for pds_default_send_message");
			}elsif ($args[1] =~ /^\/[[:graph:]]+$/){
				&setConfigParameter("PDS_Default_Send_Message", $args[1]);
			}else{
				&config_error("$args[1] is not a valid mesage");
			}
		}
    ...
    # added for http request dynamic parameter match
    # store $http_dynamic_paratoname and $http_dynamic_paratomimetype
    &setConfigHash("HTTP_Dynamic_ParaToName", %http_dynamic_paratoname);
    &setConfigHash("HTTP_Dynamic_ParaToMIMETYPE", %http_dynamic_paratomimetype);
    
    # added for url redirection <2017-04-01>
    # store http request url redirection configuration
    &setConfigHash("HTTP_Redirect_URL_List", %http_redirect_url_list);
    ...
    # added for https request dynamic parameter match
    # store $https_dynamic_paratoname and $http_dynamic_paratomimetype
    &setConfigHash("HTTPS_Dynamic_ParaToName", %https_dynamic_paratoname);
    &setConfigHash("HTTPS_Dynamic_ParaToMIMETYPE", %https_dynamic_paratomimetype);
    ...
    # added for port detection service <2017-04-11>
    # store pds_remote_tcp_port_list and pds_remote_udp_port_list
    &setConfigArray("PDS_Remote_TCP_Port_List", @pds_remote_tcp_port_list);
    &setConfigArray("PDS_Remote_UDP_Port_List", @pds_remote_udp_port_list);
    ...
    # add for port detection service  <2017-04-11>
    &setConfigParameter("PDS_ServiceName", "pds_" . &getConfigParameter("PDS_Default_Bind_Port") . "_tcp/udp");
...
# for port detection service
sub getConfigArray{
    my $key = shift;

    if (! defined $key) {
	# programming error -> exit
	&INetSim::error_exit("getConfigArray() called without parameter.");
    }elsif(exists $ConfigOptions{$key}) {
    	return @{$ConfigOptions{$key}};
    }else{
    	&INetSim::error_exit("No such configuration parameter '$key'.");
...
sub setConfigArray{
	my ($key, @values) = @_;
	if (! defined $key){
		&INetSim::error_exit("setConfigArray() called without key parameter.");
	}elsif (exists $ConfigOptions{$key}){
		$ConfigOptions{$key} = [@values];
	}else{
		&INetSim::error_exit("No such configuration option '$key'.");
